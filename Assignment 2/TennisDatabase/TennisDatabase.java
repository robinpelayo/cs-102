...
public class TennisDatabase implements TennisDatabaseInterface{
   ...
   private ObservableList<TennisPlayer> obsListTPC;   //Observable lists for the GUI
   private ObservableList<TennisMatch> obsListTMC;
   // Constructor.
   public TennisDatabase() {
      ...
      obsListTPC = FXCollections.observableArrayList();  // Initialize observable lists
      obsListTMC = FXCollections.observableArrayList();
   }
   
   public ObservableList<TennisPlayer> getObservableListPlayers() { return this.obsListTPC; }
   public ObservableList<TennisMatch> getObservableListMatches() { return this.obsListTMC; }
   ...
   public void importFromFile( String fileName ) throws TennisDatabaseRuntimeException {
      ...
      catch( FileNotFoundException e ) { System.out.println( "FileNptFoundException thrown in TDB loadFromFile" ); }
   ...
   public void exportToFile( String fileName ) throws TennisDatabaseRuntimeException {
      //Create new file writers and print writers
      try{
         FileWriter fileWriter = new FileWriter(fileName,false);
         PrintWriter printWriter = new PrintWriter(fileWriter);
         //Create new players iterator and set in pre-order traversal
         ...
         while( matchIter.hasNext() ) {
            ...
            printWriter.print( currMatch.getDateYear() );
            printWriter.print( currMatch.getDateMonth() );
            printWriter.print( currMatch.getDateDay() + "/" );
   ...
   public void deletePlayer( String playerId ) throws TennisDatabaseException {
      ...
      try{
         //Get the player to be removed for the observable list 
         TennisPlayer removePlayer = tpc.getPlayer ( playerId );
         obsListTPC.remove(removePlayer);
         //Ask the matches container and the players container to remove the matches of the player
         tmc.deleteMatchesOfPlayer( playerId );
         tpc.deleteMatchesOfPlayer( playerId );
         //Delete the player from the tree
